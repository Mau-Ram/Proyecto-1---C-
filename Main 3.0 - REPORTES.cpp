...
struct Movimiento{
    ...
    string mesString; //luego se convertiran a int para validar rangos y validez de la fechaf
    string anioString;
    string descripcion; 
    string categoria;   
    long double monto;       // Es como un float pero con números más grandes
...
//FUNCION PARA Calcular el Balance General en un mes y año específicos 
//-------------------------------------------------------------------------------------------------------------------------------
long double CalcularBalanceGeneral(Movimiento*& cabeza,string mes,string anio){
    if (cabeza == nullptr){
        ...
        cout << "ESTE ES EL BALANCE GENERAL: "<<balance_General;
...
//FUNCION PARA IMPRIMIR LOS AÑOS QUE EXISTEN EN CIERTO MES 
//-------------------------------------------------------------------------------------------------------------------------------
void Imprimir_Anios(Movimiento*& cabeza,string mes){
    // Imprimir solo los años que existen en ese mes
    Movimiento* actual = cabeza;        // variable que recorre la lista enlazada
    int contador = 1;
    string anio_temporal;

    if (cabeza == nullptr){
        return;                 // La lista esta vaciía
    } else {
        cout << "Actualmente existen los siguientes años:\n";
        cout<<"\n------------------------------------------\n";
        while (actual != nullptr){
            if (actual -> mesString == mes){
                cout << contador << ".) "<< actual ->anioString << " ";        // mostrar los años que hay dentro del mes del parametro
                contador++; // incremento en el contador
            }
            actual=actual->sgte;
        }
        cout<<"\n------------------------------------------\n";
...
//FUNCION PARA DETERMINAR SI EXISTE CIERTO MES EN NUESTRO TXT
//-------------------------------------------------------------------------------------------------------------------------------
...
//FUNCION PARA DETERMINAR SI EXISTE CIERTO AÑO EN NUESTRO TXT
//-------------------------------------------------------------------------------------------------------------------------------
...
//FUNCIÓN PARA INGRESAR EL AÑO CORRESPONDIENTE A CIERTO MES PARA LUEGO HACER UN REPORTE DE ESE MES Y AÑO
//-------------------------------------------------------------------------------------------------------------------------------
string IngresarAnio(string mes){
    string anio_temp;
    while (true) {
        cout << "\n Escribe un año de los mostrados para el reporte: ";
        cin >> anio_temp;
...
bool EsVacio(string texto){
    if (texto.empty()){
        ...
        for (char letra : texto){    // Recorre el texto letra por letra
            if (!isspace(letra)){   // Si hay al menos una letra sin espacio significa que no esta vacía
                return false;
            }
        }
    }
...
//FUNCION QUE CONVIERTE CUALQUIER NUMERO A POSITIVO
//-------------------------------------------------------------------------------------------------------------------------------
...
void AbrirTXT(Movimiento*& cabeza){
    ...
    while (fin_admin >> diaTemp >> mesTemp >> aniosTemp >> tipoTemp >> montoTemp >> categoriaTemp >>descripcionTemp){
        ...
        if (tipoTemp == 0){
            nuevo->tipo = INGRESO;
            // Agregar monto
            nuevo->monto = Positivo(montoTemp);
        } else if(tipoTemp == 1){
            nuevo->tipo = GASTO;
            montoTemp = Positivo(montoTemp);    // lo pasamos primero a positivo
            // Agregar monto
...
int GuardarTXT(Movimiento*& cabeza){
    ...
    ofstream fout_usario ("movimientos.txt");   // texto con formato
    ...
    if (!fout_usario){
        ...
        if (cabeza == nullptr){
            ...
            fout_usario << "Moviemiento #" << contador << "\n\n";
            
            // Imprimir Fecha
            fout_usario << "Fecha: " << actual->diaString << "/" << actual->mesString << "/" << actual->anioString <<"\n\n";
        
            // Tipo de Movimiento
            fout_usario << "Tipo de Movimiento: ";
            if (actual -> tipo == INGRESO){
                fout_usario << "Ingreso\n\n";
            }else if (actual -> tipo == GASTO){
                fout_usario << "Gasto\n\n";
            ...
            // Monto
            fout_usario << "Monto: " << actual->monto ;
            fout_usario << "\n\n";
            // Categoría
            fout_usario << "Categoría: "<< actual->categoria ;
            fout_usario << "\n\n";
            // Descripción
            fout_usario << "Descripción: "<< actual->descripcion ;
            fout_usario << "\n\n";
            // Fin
            fout_usario << "\n---------------------------------------------------------------------------------------\n\n";
            ...
            fout_usario.close(); //cerramos el archivo dirigido al usuario
...
void CrearMovimiento(Movimiento*& cabeza){
    ...
    while (true){
        ...
        if (EsVacio(categoria_temp)){
            cout << "\nERROR ⚠️\n Atención la categoría está vacía❗. Escribe la categoría nuevamente.\n";
        } else{
...
void Reportes(Movimiento*& cabeza){
    ...
    while(true){
        int opcion;
        cout<<"Reportes y Estadística 🏧\n";
        cout<<"\n------------------------------------------\n";
        cout<<"\n  Seleccione el mes para el que desea generar un reporte  \n";
        cout<<"1.) ENERO\n";
        cout<<"2.) FEBRERO\n";
        cout<<"3.) MARZO\n";
        cout<<"4.) ABRIL\n";
        ...
        switch (opcion){ //se declara un switch para los caminos del menu
            ...
            case 1:
                if (!ExisteElMes(lista,"01")){
                    ...
                    Imprimir_Anios(lista,"01");  // Imprimir los años que existen en los movimientos de enero

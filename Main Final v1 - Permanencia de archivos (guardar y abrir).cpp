...
// Función para intercambiar los espacios por "_"
...
// Función para separar por espacios y no por "_"
...
// función que convierte un número cualquiera en positivo
...
// Función para guardar el las listas en un txt - Van a ser dos listas una donde se guarde todo con formato y otra donde solo se guarde los datos
int GuardarTXT(Movimiento*& cabeza){
    ...
    if (!fout_usario){
        cerr<< "*** Error *** \nNo fue posible abrir el archivo: 'movimientos.txt'.\n";
        return 1; // codigo error
    } else if (!fout_admin){
        cerr<< "*** Error *** \nNo fue posible abrir el archivo: 'datos.txt'.\n";
        return 1;
...
void AbrirTXT(Movimiento*& cabeza){
    ...
    if (!fin_admin){
        cerr << "*** ERROR ***\nNo se encontró el archivo.....\n";      // Salirse de la función si no se logra abrir el archivo
        return; 
    }   

    // Variables
    string diaTemp, mesTemp,aniosTemp,categoriaTemp,descripcionTemp;
    int tipoTemp, montoTemp;

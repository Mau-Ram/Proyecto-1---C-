...
#include <set> // para verificar si las categorías se repiten (evitar duplicados en reportes)
#include <map> // Para crear diccionarios como en Python (clave-valor) [categoria-monto] en los reportes 
using namespace std; 
//-------------------------------------------------------------------------------------------------------------------------------
...
void GuardarReporteTXT(Movimiento*& cabeza, string mes, string anio){
    ...
    fout_reporte << fixed << setprecision(2); //para arreglar el formato de salida del reporte
    ...
    if (!fout_reporte){ // validar que se pueda abrir el archivo
        ...
        if (cabeza == nullptr){
            ...
            map<string,long double> Montos_por_Categoria;   // Se crea un diccionario cuya clave es un string y el valor es un long double. La clave es la categoría y el valor el monto. También funciona como set entonces no van a crearse categorías repetidas

            //Impresiones
            fout_reporte << "REPORTE " << mes_letras << " DE "<< anio<< "\n";
            fout_reporte << "---------------------------------------------------------------------------------------\n\n";
            fout_reporte << "*GASTOS DETALLADOS DEL MES\n\n";
                ...
                if (actual -> mesString == mes){    // Se toma en cuenta solamente los nodos que incluyan el mes y año seleccionados por el usuario
                    if (actual ->anioString == anio ){        
                        if (actual -> tipo == GASTO){  
                            // En este punto ya verificamos que estamos en el nodo con el mismo mes y año y además que sea un gasto

                            // Sumar todos los mondos de las categorías
                            Montos_por_Categoria[actual->categoria] += actual->monto;  // El [] contienee la categoría (clave) el += el valor
                        }
                    }   
                }
                actual=actual->sgte; // Avanzar al siguiente nodo
            ...
            // Luego de recorrer toda la lista imprimimos en el TXT solamente los valores que no están repetidos. Es decir los del map.

            for (const pair<string,long double>& categoria : Montos_por_Categoria){  // El & significa que va a tomar la dirección de memoria original de lo que haya en categoria // Los dos puntos en el for ":" significa que va a recorrer el diccionario de principio a fin

                fout_reporte << "Categoría #"<< contador_categoria << ": "<< categoria.first << "\n\n"; // .first es el primer elemento del par (llave,valor) entocnces agrega la categoria al txt
                fout_reporte << "Monto total gastado: "<< categoria.second << "\n\n";

                contador_categoria++; // Se incrementan el contador solo si la categoría es nueva

            }

                fout_reporte << "---------------------------------------------------------------------------------------\n\n";
                fout_reporte << "*ESTADISTICAS GENERALES\n\n";
